...
public class DrawingView extends View {
	...
	ShapeContainer selectedShapesContainer = new ShapeContainer();
	...
	// This is only used when one or multiple shapes are selected with the lasso, otherwise it is equal to -1
	int indexOfSelectionBeingManipulated = -1;
					...
					switch ( currentMode ) {
						case MODE_NEUTRAL :
							if ( cursorContainer.getNumCursors() == 1 && type == MotionEvent.ACTION_DOWN ) {
								...
								indexOfSelectionBeingManipulated = selectedShapesContainer.indexOfShapeContainingGivenPoint( p_world );
						...
						case MODE_SHAPE_MANIPULATION :
							...
							if ( cursorContainer.getNumCursors()>0 && type == MotionEvent.ACTION_MOVE && ( indexOfShapeBeingManipulated>=0 || indexOfSelectionBeingManipulated>=0 )) {
						...
						case MODE_LASSO :
							if ( type == MotionEvent.ACTION_DOWN ) {
								...
								if ( cursor.getType() == MyCursor.TYPE_DRAGGING ) {
									...
									selectedShapesContainer.empty_container();
						...
						case MODE_ERASE :
							...
							if ( type == MotionEvent.ACTION_DOWN ) {
								if(indexOfShapeBeingManipulated >= 0) {
									...
									selectedShapes.clear();
								} else if (eraseButton.contains(point_x_y) ) {
									erase_on = 0;
									currentMode = MODE_NEUTRAL;
								}
							}
	...
	public void createShapeFromSelection(){
		...
		points = Point2DUtil.computeConvexHull( points );
		selectedShapesContainer.addShape(points);
	}
